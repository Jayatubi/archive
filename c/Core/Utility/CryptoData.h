...
public:
    ...
    CryptoData():m_pData(GX_MemoryPoolNew(T))
    {
    }
    
    ~CryptoData()
    {
        m_pData ? GX_MemoryPoolDelete(m_pData) : (void)0;
    }
    
    template<typename TData>
    CryptoData(const TData & rData):m_pData(GX_MemoryPoolNew(T))
    {
        *m_pData = crypto((TData)rData);
    }
    
    CryptoData(const tData& data):m_pData(GX_MemoryPoolNew(T))
    {
        *m_pData = crypto(data);
    }
    
    CryptoData(const CryptoData<tData>& r):m_pData(GX_MemoryPoolNew(T))
    {
        *m_pData = *(r.m_pData);
    }
...
namespace GX
{
    ...
    {
        template<typename T>
        struct ToStringValueCalc<CryptoData<T>>
        {
            typedef typename CryptoData<T>::tData ValueType;
        };
    };
